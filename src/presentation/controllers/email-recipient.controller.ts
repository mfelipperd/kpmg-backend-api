...
} from "../../application/dtos/email-recipient.dto";
...
export class EmailRecipientController {
  ...
  async create(@Body() createEmailRecipientDto: CreateEmailRecipientDto) {
    const recipient = await this.createEmailRecipientUseCase.execute(
      createEmailRecipientDto,
    );
  ...
  @ApiResponse({
    status: 200,
    description: "Lista de destinatários retornada com sucesso",
  })
  async findAll() {
    const recipients = await this.getEmailRecipientsUseCase.execute();
    return recipients.map((recipient) =>
      EmailRecipientResponseDto.fromEntity(recipient),
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Destinatário deletado com sucesso",
  })
  @ApiResponse({ status: 404, description: "Destinatário não encontrado" })
