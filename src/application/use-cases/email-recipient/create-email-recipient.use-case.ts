...
export class CreateEmailRecipientUseCase {
  ...
  async execute(request: CreateEmailRecipientRequest): Promise<EmailRecipient> {
    ...
    const existingRecipient = await this.emailRecipientRepository.findByEmail(normalizedEmail);
    if (existingRecipient) {
      throw new ConflictException('Este e-mail já está cadastrado');
    }

    const emailRecipient = EmailRecipient.create(normalizedEmail, request.active);
    const savedRecipient = await this.emailRecipientRepository.create(emailRecipient);
